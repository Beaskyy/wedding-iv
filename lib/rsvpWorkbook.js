...
const RSVP_TIMEZONE = 'America/Port_of_Spain'
...
function formatTimestamp(date = new Date()) {
  const day = parseInt(new Intl.DateTimeFormat('en-US', { timeZone: RSVP_TIMEZONE, day: 'numeric' }).format(date), 10)
  const month = new Intl.DateTimeFormat('en-US', { timeZone: RSVP_TIMEZONE, month: 'long' }).format(date)
  const year = new Intl.DateTimeFormat('en-US', { timeZone: RSVP_TIMEZONE, year: 'numeric' }).format(date)
  ...
  return `${day}${getOrdinalSuffix(day)} ${month} ${year}, ${time} (GMT-4)`
}
